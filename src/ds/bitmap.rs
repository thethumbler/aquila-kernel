...
pub unsafe fn bitmap_size(n: usize) -> usize {
    return (n + BITMAP_BLOCK_MASK) / BITMAP_BLOCK_SIZE * core::mem::size_of::<bitmap_t>();
}

pub unsafe fn bitmap_set(bitmap: *mut BitMap, index: usize) -> () {
    *(*bitmap).map.offset(bitmap_block_offset!(index) as isize) |= 1 << bitmap_bit_offset!(index);
}

pub unsafe fn bitmap_clear(bitmap: *mut BitMap, index: usize) -> () {
    *(*bitmap).map.offset(bitmap_block_offset!(index) as isize) &= !(1 << bitmap_bit_offset!(index));
}

pub unsafe fn bitmap_check(bitmap: *mut BitMap, index: usize) -> isize {
    return (*(*bitmap).map.offset(bitmap_block_offset!(index) as isize) & (1 << bitmap_bit_offset!(index))) as isize;
}

pub unsafe fn bitmap_set_range(bitmap: *mut BitMap, findex: usize, lindex: usize) -> () {
    let mut i = findex;
...
pub unsafe fn bitmap_clear_range(bitmap: *mut BitMap, findex: usize, lindex: usize) -> () {
    let mut i = findex;
